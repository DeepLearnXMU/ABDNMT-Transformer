...
def abd_get_training_input(filenames, params):
    """ Get input for training stage
    ...
    with tf.device("/cpu:0"):
        src_dataset = tf.data.TextLineDataset(filenames[0])
        tgt_dataset = tf.data.TextLineDataset(filenames[1])
        r2l_tgt_dataset = tf.data.TextLineDataset(filenames[2])

        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, r2l_tgt_dataset))
        ...
        # Split string
        dataset = dataset.map(
            lambda src, tgt, r2l_tgt: (
                tf.string_split([src]).values,
                tf.string_split([tgt]).values,
                tf.string_split([r2l_tgt]).values,
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.map(
            lambda src, tgt, r2l_tgt: (
                tf.concat([src, [tf.constant(params.eos)]], axis=0),
                tf.concat([tgt, [tf.constant(params.eos)]], axis=0),
                tf.concat([r2l_tgt, [tf.constant(params.eos)]], axis=0),
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.map(
            lambda src, tgt, r2l_tgt: {
                ...
                "source_length": tf.shape(src),
                "target_length": tf.shape(tgt),
                "r2l_target_length": tf.shape(r2l_tgt),
            },
            num_parallel_calls=params.num_threads
        )
        ...
        features["source"] = src_table.lookup(features["source"])
        features["target"] = tgt_table.lookup(features["target"])
        features["r2l_target"] = tgt_table.lookup(features["r2l_target"])
        ...
        features["source"] = tf.to_int32(features["source"])
        features["target"] = tf.to_int32(features["target"])
        features["r2l_target"] = tf.to_int32(features["r2l_target"])
        features["source_length"] = tf.to_int32(features["source_length"])
        features["target_length"] = tf.to_int32(features["target_length"])
        features["r2l_target_length"] = tf.to_int32(features["r2l_target_length"])
        features["source_length"] = tf.squeeze(features["source_length"], 1)
        features["target_length"] = tf.squeeze(features["target_length"], 1)
        features["r2l_target_length"] = tf.squeeze(features["r2l_target_length"], 1)
...
def abd_sort_input_file(filenames, reverse=True):
    # Read file
    with tf.gfile.Open(filenames[0]) as fd:
        inputs0 = [line.strip() for line in fd]
    with tf.gfile.Open(filenames[1]) as fd:
        inputs1 = [line.strip() for line in fd]

    input_lens = [
        (i, len(line.strip().split())) for i, line in enumerate(inputs0)
    ]
    ...
    for i, (index, _) in enumerate(sorted_input_lens):
        sorted_inputs.append((inputs0[index], inputs1[index]))
        sorted_keys[index] = i
...
def abd_get_evaluation_input(inputs, params):
    with tf.device("/cpu:0"):
        # Create datasets
        datasets = []
        ...
        dataset = dataset.map(
            lambda *x: {
                ...
                "source_length": tf.shape(x[0])[0],
                "r2l_target": x[1],
                "r2l_target_length": tf.shape(x[1])[0],
                "references": x[2:]
            },
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.padded_batch(
            params.eval_batch_size,
            {
                "source": [tf.Dimension(None)],
                "source_length": [],
                "r2l_target": [tf.Dimension(None)],
                "r2l_target_length": [],
                "references": (tf.Dimension(None),) * (len(inputs) - 2)
            },
            {
                "source": params.pad,
                "source_length": 0,
                "r2l_target": params.pad,
                "r2l_target_length": 0,
                "references": (params.pad,) * (len(inputs) - 2)
            }
        )
...
def get_inference_input(inputs, params):
    ...
    with tf.device("/cpu:0"):
        ...
        return features
def abd_get_inference_input(inputs, params):
    if params.generate_samples:
        batch_size = params.sample_batch_size
    else:
        batch_size = params.decode_batch_size
