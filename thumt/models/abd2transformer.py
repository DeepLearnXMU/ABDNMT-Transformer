...
def transformer_decoder(inputs, memory, bias, mem_bias, params, state=None,
    ...
    with tf.variable_scope(scope, dtype=dtype,
        ...
        if state is not None:
            return (outputs, next_state),None

        return outputs, atn_weights
...
def r2l_decoder(decoder_input, encoder_output, dec_attn_bias, enc_attn_bias, params):
    ...
    if params.share_r2l:
        scope = 'decoder'
    r2l_state, _ = transformer_decoder(decoder_input, encoder_output, dec_attn_bias, enc_attn_bias, params, scope=scope)
    return r2l_state


def l2r_decoder(r2l_mem, r2l_attn_bias, decoder_input, encoder_output, dec_attn_bias, enc_attn_bias, params,
                state=None):
    scope = 'l2r'
    if params.share_r2l:
        scope = 'decoder'
    outputs, atn_weights = transformer_decoder(decoder_input, encoder_output, dec_attn_bias, enc_attn_bias, params,
                                               state,
                                               scope=scope, r2l_memory=r2l_mem, r2l_bias=r2l_attn_bias,
                                               reuse=params.share_r2l)
    return outputs, atn_weights


def abd_forward(r2l_input, r2l_bias, l2r_input, l2r_bias, encoder_output, enc_attn_bias, r2l_attn_bias, params,
                state=None, lens_r2l=None, lens_l2r=None):
    ...
    l2r_outputs, atn_weights = l2r_decoder(r2l_mem, r2l_attn_bias, l2r_input, encoder_output, l2r_bias, enc_attn_bias,
                                           params, state)

    if state is None:
        atn_weights = tf.concat(atn_weights, axis=1)  # B x HL x T_l2r x T_r2l
        l_r2l = lens_r2l
        l_l2r = lens_l2r

        l_l2r_max = tf.reduce_max(l_l2r)
        l_r2l_max = tf.reduce_max(l_r2l)
        tmp = tf.reshape(tf.range(l_l2r_max), (1, l_l2r_max))

        # pos=tf.maximum(3,tf.reshape(l_r2l,(B,1,1,1))-tf.tile(tmp,[1,H*num_layer,1,1]))
        pos = tf.maximum(20, tf.reshape(l_r2l, (-1, 1)) - tmp)  # B x T_l2r

        mask = tf.to_float(tf.sequence_mask(pos, l_r2l_max))  # B x T_l2r x T_r2l
        mask = tf.expand_dims(mask, 1)  # B x 1 x T_l2r x T_r2l

        positive = tf.reduce_mean(atn_weights * mask, 1)  # B x T_l2r x T_r2l
        negative = tf.reduce_mean(atn_weights * (1.0 - mask), 1)  # B x T_l2r x T_r2l

        eps = 1e-6
        positive = tf.reduce_sum(positive) / (tf.reduce_sum(mask) + eps)
        negative = tf.reduce_sum(negative) / (tf.reduce_sum(1.0 - mask) + eps)

        atn_loss = -(positive - negative)

        return r2l_mem, l2r_outputs, atn_loss
...
def decoding_graph(features, state, mode, params):
    ...
    if mode == "train":
        if params.atn_loss > 0:
            r2l_output, l2r_output, atn_loss = abd_forward(r2l_decoder_input, r2l_bias, decoder_input, dec_attn_bias,
                              encoder_output, enc_attn_bias, r2l_attn_bias, params,
                              lens_r2l=r2l_tgt_len, lens_l2r=tgt_len)
    ...
    if params.atn_loss > 0:
        reg_term = tf.maximum(-0.05, atn_loss) * params.atn_loss
        # reg_term=atn_loss * params.atn_loss
        return l2r_loss + r2l_loss + reg_term
    else:
