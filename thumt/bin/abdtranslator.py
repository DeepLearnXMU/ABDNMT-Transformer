...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        # Read input file
        sorted_keys, sorted_inputs = dataset.abd_sort_input_file(args.input)
        # Build input queue
        features = dataset.abd_get_inference_input(sorted_inputs, params)
        # Create placeholders
        placeholders = []
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                ...
                "source_length": tf.placeholder(tf.int32, [None],
                                                "source_length_%d" % i),
                "r2l_target": tf.placeholder(tf.int32, [None, None],
                                         "r2l_target_%d" % i),
                "r2l_target_length": tf.placeholder(tf.int32, [None],
                                                "r2l_target_length_%d" % i),
            })
        ...
        sorted_inputs=[(src,r2l) for src,r2l in zip(*sorted_inputs)]

        for index in range(len(sorted_inputs)):
            restored_inputs.append(sorted_inputs[sorted_keys[index]][0])
