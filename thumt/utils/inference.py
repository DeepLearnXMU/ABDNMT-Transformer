...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        if 'r2l_target' in features:
            local_features = {
                "source": features["source"],
                "source_length": features["source_length"],
                "r2l_target": features["r2l_target"],
                "r2l_target_length": features["r2l_target_length"],
                # [bos_id, ...] => [..., 0]
...
def create_inference_graph(models, features, params):
    ...
    if 'r2l_target' in features:
        _init_seqs = tf.fill([batch_size,  1], bos_id)
        _init_lens=tf.to_int32(tf.fill([batch_size], 1))
        features['target']=_init_seqs
        features['target_length']=_init_lens
    ...
    features["source"] = tf.reshape(features["source"],[shape[0] * shape[1], shape[2]])

    # For source sequence length
    features["source_length"] = tf.expand_dims(features["source_length"], 1)
    features["source_length"] = tf.tile(features["source_length"],[1, beam_size])
    ...
    # [batch, beam_size, length] => [batch * beam_size, length]
    features["source_length"] = tf.reshape(features["source_length"],[shape[0] * shape[1]])

    if 'r2l_target' in features:
        features["r2l_target"] = tf.expand_dims(features["r2l_target"], 1)
        features["r2l_target"] = tf.tile(features["r2l_target"], [1, beam_size, 1])
        r2l_shape = tf.shape(features["r2l_target"])
        features["r2l_target"] = tf.reshape(features["r2l_target"], [r2l_shape[0] * r2l_shape[1], r2l_shape[2]])
        features["r2l_target_length"] = tf.expand_dims(features["r2l_target_length"], 1)
        features["r2l_target_length"] = tf.tile(features["r2l_target_length"],[1, beam_size])
        r2l_shape = tf.shape(features["r2l_target_length"])
        features["r2l_target_length"] = tf.reshape(features["r2l_target_length"],[r2l_shape[0] * r2l_shape[1]])

        del features['target']
        del features['target_length']
