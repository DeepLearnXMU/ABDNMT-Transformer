...
def _evaluate(eval_fn, input_fn, decode_fn, path, config):
    ...
    with graph.as_default():
        ...
        if 'r2l_target' in features:
            placeholders['r2l_target']=tf.placeholder(tf.int32, [None, None], "r2l_target")
            placeholders['r2l_target_length']=tf.placeholder(tf.int32, [None], "r2l_target_length")
            ...
            while not sess.should_stop():
                ...
                if 'r2l_target' in features:
                    outputs = sess.run(predictions, feed_dict={
                        ...
                        placeholders["r2l_target"]: feats["r2l_target"],
                        placeholders["r2l_target_length"]: feats["r2l_target_length"],
                    })
                else:
                    outputs = sess.run(predictions, feed_dict={
                        placeholders["source"]: feats["source"],
